...
export default function AuditTab() {
  ...
  const [subTab, setSubTab] = useState<"chat" | "quiz" | "kahoot" | "classroom">("chat");
  const [filterText, setFilterText] = useState<string>("");

  // Dual-Database State
  const [activeDbInfo, setActiveDbInfo] = useState(() => {
    return typeof window !== "undefined" && localStorage.getItem("chem_active_supabase_mode") === "primary"
      ? { mode: "primary", label: "Supabase Chính (cohutjbyyubjntqhjoao)", isBackup: false }
      : { mode: "backup", label: "Supabase Dự Phòng (gxmwmevxfyzgnkcrlrjkdi)", isBackup: true };
  });

  // Classroom Management State
  const [studentList, setStudentList] = useState<any[]>([]);
  const [selectedStudentForBadge, setSelectedStudentForBadge] = useState<any | null>(null);
  ...
  const handleSwitchDb = () => {
    const nextMode = activeDbInfo.mode === "backup" ? "primary" : "backup";
    if (typeof window !== "undefined") {
      localStorage.setItem("chem_active_supabase_mode", nextMode);
    }
    const updated = nextMode === "backup"
      ? { mode: "backup", label: "Supabase Dự Phòng (gxmwmevxfyzgnkcrlrjkdi)", isBackup: true }
      : { mode: "primary", label: "Supabase Chính (cohutjbyyubjntqhjoao)", isBackup: false };
    setActiveDbInfo(updated);
    fetchCurrentSubData(subTab);
  };
    ...
    try {
      if (type === "chat") {
        ...
        if (data && data.length > 0) {
          ...
          try {
            const raw = localStorage.getItem("chem_student_accounts_v3");
            setStudentList(raw ? JSON.parse(raw) : []);
          } catch {
            setStudentList([]);
          }
        }
    ...
    >
      <CardContent sx={{ p: { xs: 1.5, sm: 2, md: 3 } }}>
        ...
        {!isAuthed ? (
          ...
          <Box>
            {/* DUAL-DATABASE STATUS & SWITCHER BAR */}
            <Paper sx={{ p: 1.5, mb: 2, bgcolor: '#0f172a', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: 2, display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 1 }}>
              <Box display="flex" alignItems="center" gap={1}>
                <span className={`w-2.5 h-2.5 rounded-full ${activeDbInfo.isBackup ? 'bg-emerald-400 animate-pulse' : 'bg-cyan-400'}`} />
                <Typography variant="caption" color="text.secondary">
                  CSDL Đang Kết Nối: <strong style={{ color: activeDbInfo.isBackup ? '#34d399' : '#38bdf8' }}>{activeDbInfo.label}</strong>
                </Typography>
                <Chip
                  label={activeDbInfo.isBackup ? "Dự Phòng Hoạt Động" : "Chính"}
                  size="small"
                  color={activeDbInfo.isBackup ? "success" : "primary"}
              ...
              <Button
                ...
                sx={{ textTransform: 'none', fontSize: '11px', color: '#f59e0b', borderColor: 'rgba(245, 158, 11, 0.4)' }}
              >
                Chuyển Sang {activeDbInfo.isBackup ? "CSDL Chính" : "CSDL Dự Phòng"}
              </Button>
            </Paper>
            ...
            >
              <Stack direction="row" spacing={1} flexWrap="wrap">
                ...
                <Button
                  size="small"
                  variant={subTab === "classroom" ? "contained" : "outlined"}
                  color="warning"
                  onClick={() => {
                    setSubTab("classroom");
                    fetchCurrentSubData("classroom");
                  }}
            ...
            {subTab === "classroom" && (
              ...
              >
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} flexWrap="wrap" gap={1}>
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    <Typography variant="subtitle1" fontWeight="bold" color="common.white">
                      Danh Sách Học Sinh Trong Hệ Thống ({studentList.length} Học Sinh)
                    </Typography>
                  </Box>
                </Box>
                ...
                <Table size="small">
                  <TableHead sx={{ bgcolor: "#1e293b" }}>
                    <TableRow>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold", width: 50 }}>STT</TableCell>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold" }}>Học Sinh</TableCell>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold" }}>Lớp & Trường</TableCell>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold", textAlign: 'center' }}>Chuỗi Học (Streak)</TableCell>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold", textAlign: 'center' }}>Điểm EXP</TableCell>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold" }}>Danh Hiệu GV Gắn</TableCell>
                      <TableCell sx={{ color: "#f59e0b", fontWeight: "bold", textAlign: 'right' }}>Thao Tác Giáo Viên</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {studentList.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={7} align="center" sx={{ color: "text.secondary", py: 3 }}>
                          Chưa có tài khoản học sinh nào trong cơ sở dữ liệu.
                        </TableCell>
                      </TableRow>
                    ) : (
                      studentList
                        .filter(s => !filterText || (s.name || s.username || "").toLowerCase().includes(filterText.toLowerCase()) || (s.class_name || "").toLowerCase().includes(filterText.toLowerCase()))
                        .map((student, idx) => (
                          <TableRow key={student.id || idx} sx={{ '&:hover': { bgcolor: 'rgba(255,255,255,0.03)' } }}>
                            <TableCell sx={{ color: 'text.secondary', fontSize: 12 }}>{idx + 1}</TableCell>
                            <TableCell>
                              <Typography variant="body2" fontWeight="bold" color="common.white" sx={{ fontSize: 13 }}>
                                {student.name || student.username || "Học sinh"}
                              </Typography>
                            ...
                            <TableCell>
                              <Chip label={student.class_name || "10A1"} size="small" sx={{ height: 20, fontSize: 10, bgcolor: 'rgba(56, 189, 248, 0.15)', color: '#38bdf8', mr: 1 }} />
                              <Typography variant="caption" color="text.secondary">
                                {student.school_name || "THPT Chuyên"}
                              </Typography>
                            </TableCell>
                            <TableCell align="center">
                              <Chip icon={<Flame size={12} />} label={`${student.login_streak || student.streak || 1} ngày`} size="small" sx={{ height: 20, fontSize: 10, bgcolor: 'rgba(249, 115, 22, 0.15)', color: '#fb923c', fontWeight: 'bold' }} />
                            </TableCell>
                            <TableCell align="center">
                              <Typography variant="body2" fontWeight="bold" color="#facc15" fontFamily="monospace">
                                {student.kahoot_exp || student.exp || 100} EXP
                              </Typography>
                            </TableCell>
                            <TableCell>
                              <Chip
                                label={student.teacher_badge || "Chưa gắn"}
                                size="small"
                                sx={{
                                  height: 22,
                                  fontSize: 11,
                                  bgcolor: student.teacher_badge ? 'rgba(234, 179, 8, 0.2)' : 'rgba(255,255,255,0.05)',
                                  color: student.teacher_badge ? '#facc15' : '#94a3b8',
                                  fontWeight: student.teacher_badge ? 'bold' : 'normal'
                                }}
                              />
                            </TableCell>
                              ...
                              <Stack direction="row" spacing={1} justifyContent="flex-end">
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setCustomBadgeName(student.teacher_badge || "");
        ...
        {/* DIALOG GÁN DANH HIỆU CUSTOM CỦA GIÁO VIÊN */}
        <Dialog
          open={openBadgeModal}
        ...
        >
          <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, color: '#f59e0b', fontWeight: 'bold' }}>
            <Award size={20} />
            Gán Danh Hiệu Cho Học Sinh
          </DialogTitle>
          <DialogContent>
            <Typography variant="body2" color="text.secondary" mb={2}>
              Học sinh: <strong style={{ color: '#fff' }}>{selectedStudentForBadge?.name || selectedStudentForBadge?.username}</strong> ({selectedStudentForBadge?.class_name || "Lớp 10A1"})
            </Typography>

            <TextField
              label="Tên Danh Hiệu / Khen Thưởng"
              fullWidth
              size="small"
              value={customBadgeName}
              onChange={(e) => setCustomBadgeName(e.target.value)}
              placeholder="VD: Thủ Khoa Kahoot, Bậc Thầy Este, Phù Thủy Ống Nghiệm..."
              sx={{ mb: 2 }}
            />

            <Typography variant="caption" color="text.secondary" display="block" mb={1}>
              Gợi ý nhanh danh hiệu sư phạm:
            </Typography>
            <Box display="flex" flexWrap="wrap" gap={0.8}>
              {["Thủ Khoa Kahoot", "Phù Thủy Ống Nghiệm", "Bậc Thầy Este", "Ngôi Sao Halogen", "Chiến Binh Chăm Chỉ"].map((badge, idx) => (
                <Chip
                  key={idx}
                  label={badge}
                  size="small"
                  onClick={() => setCustomBadgeName(badge)}
                  sx={{ cursor: 'pointer', bgcolor: 'rgba(245, 158, 11, 0.15)', color: '#facc15', fontSize: 11 }}
                />
              ))}
            </Box>
          </DialogContent>
          <DialogActions sx={{ p: 2 }}>
            <Button onClick={() => setOpenBadgeModal(false)} sx={{ color: 'text.secondary' }}>
              Hủy
            </Button>
            <Button
              variant="contained"
              ...
              onClick={async () => {
                ...
                try {
                  await supabase
                    .from("student_profiles")
                    .update({ teacher_badge: customBadgeName })
                    .eq("id", selectedStudentForBadge.id);
                  setStudentList(prev => prev.map(s => s.id === selectedStudentForBadge.id ? { ...s, teacher_badge: customBadgeName } : s));
                  setOpenBadgeModal(false);
                } catch (e: any) {
                  alert("Lỗi lưu danh hiệu: " + e.message);
                }
              }}
              sx={{ fontWeight: 'bold' }}
            >
