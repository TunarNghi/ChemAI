...
import { Trophy, Flame, Play, Users, Lock, ShieldCheck, RefreshCw, CheckCircle2, XCircle } from 'lucide-react';
...
function cleanChemicalLatex(text: string): string {
  ...
  let res = text
    ...
    .replace(/\$([^\$]+)\$/g, "$1")
    .replace(/\^\{([^}]+)\}/g, "<sup>$1</sup>")
    .replace(/\^([a-zA-Z0-9+\-\\circ]+)/g, (m, p) => `<sup>${p.replace(/\\circ/g, "o")}</sup>`)
